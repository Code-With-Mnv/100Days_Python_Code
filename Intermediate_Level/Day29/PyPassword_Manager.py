...
def save_data():
    ...
    if password == "" or username == "" or web_app == "":
        ...
        isokay = tkinter.messagebox.askokcancel(
            title="Confirmation",
            message=f"App/Website: {web_app}\nUsername: {username}\n"
            f"Password: {password}\nOkay to save?",
        )

        if isokay:
            with open("data.txt", "a") as f:
                f.write(f"{web_app} | {username} | {password} \n")
...
def find_pass():
    ...
    with open("data.txt", "r") as file:
        ...
        for i in data:
            acc_data = i.split(sep=" | ")
            if acc_data[0] == web_app:
                tkinter.messagebox.showinfo(
                    title="Account Found!",
                    message=f"Web/App: {web_app}\nUsername: {acc_data[1]}\n"
                    f"Password: {acc_data[2]}",
                )
                found = 1

    if found == 1:
        tkinter.messagebox.showerror(
            title="No More Accounts!", message=f"No more {web_app} accounts are found!"
        )
    if found == 0:
        tkinter.messagebox.showerror(
...
logo_pic = PhotoImage(file="../Day30/PyPassword_Manager/logo.png")
